...
function Nav({ href, label, icon, active }: { href: string; label: string; icon: MarketIconName; active: boolean }) { return <Link href={href} className={`market-nav flex min-h-[45px] items-center gap-3 border px-3 py-2 transition-[transform,background-color,color] duration-100 focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-[#f1c27a] ${active ? "border-[#d5ae73] bg-[#fff0ce] font-black text-[#71331f] shadow-[3px_3px_0_#2d190e]" : "border-transparent text-[#fff0ce] hover:translate-x-0.5 hover:bg-[#5b361f] hover:text-white"}`}><span className={`grid h-7 w-7 place-items-center ${active ? "text-[#b24e25]" : "text-[#f2bd50]"}`}><MarketIcon name={icon} /></span><span className="text-sm">{label}</span>{active && <span className="ml-auto text-[#d5a12d]" aria-hidden="true">★</span>}</Link>; }
