...
export default function RequestRoomPage() {
  ...
  return <div className="request-market flex h-[calc(100vh-122px)] min-h-0 w-full flex-col overflow-hidden text-[#3d281b]">
    <header className="market-entrance mb-3 flex w-full shrink-0 items-center gap-3">
    ...
    <section aria-labelledby="market-items" className="flex min-h-0 w-full flex-1 flex-col">
      <div className="mb-4 grid shrink-0 grid-cols-[minmax(0,1fr)_120px] items-center gap-3">
        ...
        <span className="w-[120px] whitespace-nowrap text-right text-sm font-black text-[#634630]">ทั้งหมด {filtered.length} รายการ</span>
      </div>

      <div aria-labelledby="market-zones" className="mb-4 shrink-0">
      ...
      <div className="product-scroll min-h-0 min-w-0 w-full flex-1 overflow-y-auto overscroll-contain [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" tabIndex={0}>
        {query.isLoading ? <LoadingMarket /> : query.isError ? <MarketError error={query.error} retry={() => query.refetch()} /> : !filtered.length ? <EmptyStatePixel title="ไม่พบสินค้า" description="ลองเปลี่ยนหมวดหมู่หรือใช้คำค้นหาอื่น" /> : <div className="market-catalog grid w-full grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
          {filtered.map((item) => {
...
function LoadingMarket() {
  return <div className="grid w-full grid-cols-1 gap-3.5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5" aria-label="กำลังโหลดสินค้า">
