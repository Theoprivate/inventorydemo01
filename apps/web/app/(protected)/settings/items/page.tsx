...
export default function ItemsPage() {
  ...
  return <>
    ...
    <GamePanel className="mb-6 flex flex-col gap-3 bg-[var(--color-game-cream-active)] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="page-market-header__eyebrow">SHARED BY EVERY BRANCH</p><p className="items-shared-note mt-1">หน้านี้ยังไม่ได้กำหนดว่าสาขาใดใช้สินค้า ให้ไปเปิดใช้และตั้งค่าในสินค้าประจำสาขา</p></div><GameButtonLink href="/settings/store-items" variant="secondary">ไปสินค้าประจำสาขา →</GameButtonLink></GamePanel>
    {success && <GamePanel role="status" className="mb-5 p-4"><StatusBadge tone="success">SAVE COMPLETE</StatusBadge><p className="items-save-message mt-2">{success}</p></GamePanel>}
    {items.isLoading ? <ItemsSkeleton /> : items.isError ? <ErrorBox error={items.error} retry={() => items.refetch()} /> : !validItems.length ? <EmptyState title="ยังไม่มีไอเทม" description="เพิ่มข้อมูลกลางของสินค้ารายการแรก" action={canEdit ? <GameButtonLink href="/settings/items/new">เพิ่มไอเทมแรก</GameButtonLink> : undefined} /> : !filtered.length ? <EmptyState title={hasFilters ? "ไม่พบไอเทมที่ค้นหา" : "ยังไม่มีไอเทม"} description={hasFilters ? "ลองเปลี่ยนคำค้นหาหรือตัวกรอง" : undefined} /> : <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {filtered.map((item) => <GameCard key={item.itemId}><div className="grid grid-cols-[112px_minmax(0,1fr)]"><ItemImage src={item.imageUrl} itemName={item.itemName} className="h-full w-full border-r border-[var(--color-game-border)]" /><div className="min-w-0 p-4"><StatusBadge className="item-card__id">{item.itemId}</StatusBadge><h2 className="item-card__name">{item.itemName}</h2><p className="item-card__metadata">{categories.data?.find((value) => value.categoryId === item.categoryId)?.categoryName || item.categoryId} · {item.unit}</p><StatusBadge className="item-card__status" tone={item.isActive ? "success" : "danger"}>{item.isActive ? "ACTIVE" : "INACTIVE"}</StatusBadge></div></div>{canEdit && <div className="border-t border-[var(--color-game-border)] p-3"><GameButtonLink className="items-edit-button" variant="secondary" href={`/settings/items/${encodeURIComponent(item.itemId)}/edit`}>แก้ไขไอเทม →</GameButtonLink></div>}</GameCard>)}
    </div>}
  </>;
}
