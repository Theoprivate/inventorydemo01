...
export default function ItemsPage() {
  ...
  return <>
    <PageHeader eyebrow="Item Catalog · Master Data" title="ตั้งค่าไอเทม" description={`จัดการชื่อ หมวดหมู่ หน่วย และรูปสินค้าของร้าน · ${validItems.length} รายการ`} actions={canEdit ? <GameButtonLink href="/settings/items/new">+ เพิ่มไอเทม</GameButtonLink> : undefined} />
    ...
    <GamePanel className="mb-6 flex flex-col gap-3 bg-[var(--color-game-cream-active)] p-4 sm:flex-row sm:items-center sm:justify-between"><div><p className="page-market-header__eyebrow">ITEM MASTER</p><p className="mt-1 text-sm font-bold">ข้อมูลกลาง เช่น ชื่อ รูป หน่วย และหมวดหมู่</p></div><GameButtonLink href="/settings/store-items" variant="secondary">ไปตั้งค่าไอเทมสาขา →</GameButtonLink></GamePanel>
